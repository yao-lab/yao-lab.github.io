...
for i = 1:Agroup_num
    ...
    sum_node = sum(sum(L(:, J1(group_tmp))));
    for j = 1:size_tmp
        sum_j_node = sum(L(:, J1(group_tmp(j))));
        Ascore(group_tmp(j)) = size_tmp/size_J1 * (sum_j_node/sum_node);
    end
end
    
[~,idA] = sort(Ascore,'descend');
% index(J1(id))
webpage{J1(idA(1:5))}
...
for i = 1:Hgroup_num
    ...
    sum_node = sum(sum(L(J2(group_tmp),:)));
    for j = 1:size_tmp
        sum_j_node = sum(L(J2(group_tmp(j)),:));
        Hscore(group_tmp(j)) = size_tmp/size_J2 * (sum_j_node/sum_node);
    end
end

[~,idH] = sort(Hscore,'descend');
webpage{J2(idH(1:5))}

dset_aut = setdiff([1:76], J1(idA));
dset_hub = setdiff([1:76], J2(idH));

idA = [J1(idA), dset_aut]; idH = [J2(idH)', dset_hub];


% spearman
spearman_corr_aut = corr(idA',rank_cn,'Type','Spearman'); % authority
spearman_corr_hub = corr(idH',rank_cn,'Type','Spearman'); % hub

% Kendall
kendall_corr_aut = corr(idA',rank_cn,'Type','Kendall'); % authority
kendall_corr_hub = corr(idH',rank_cn,'Type','Kendall'); % hub