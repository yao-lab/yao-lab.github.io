...
cov4 = cov3 - C3;
figure
for lambda = 299.419:0.0001:299.42
    cvx_begin
    variable C4(d,d) symmetric;
    C4 == semidefinite(d);
    maximize(trace(cov4*C4)-lambda*(e'*abs(C4)*e));
    subject to
    trace(C4) == 1;
    cvx_end
    target4 = trace(cov4*C4)-lambda*(e'*abs(C4)*e);
    plot(lambda,target4,'ro',lambda,eigenvalues(4),'g+')
    hold on
end
hold off
xlabel('\lambda')
ylabel('4th largest singular value')
legend('spca sval','true sval')
title('\lambda with 4th spca sval and true sval')
%we can see that lambda=299.4195 is the best for the fourth singular value 
%based on first lambda=0, second lambda=94.327 and third lambda=299.33