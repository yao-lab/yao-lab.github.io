...
format1_dir = working_folder + '/data/backtest_data_format1_week_final.h5'
format2_dir = working_folder + '/data/backtest_data_format2_week_final.h5'
...
from LuLanxi.strategy import handle_bar


class memory:
    def __init__(self):
        self.data_save = {k: pd.DataFrame(columns=['open', 'high', 'low', 'close', 'volume']) for k in
                          ASSETS_MAPPING.keys()}
        self.threshold_dict = {}

    def save_data(self, counter, data: list):
        for asset in ASSETS_MAPPING.keys():
            data_one_row = dict(zip(['open', 'high', 'low', 'close', 'volume'], data[ASSETS_MAPPING[asset]]))
            self.data_save[asset] = self.data_save[asset].append(data_one_row, ignore_index=True)
        return

    def get_threshold(self, asset, VRI_data):
        self.threshold_dict[asset] = VRI_data.quantile([0.05, 0.85])
        return
