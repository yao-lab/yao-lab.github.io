...
from LiChengxin.smart_money_factor.strategy import handle_bar
...
class memory:
    def __init__(self):
        self.data_save = {k: pd.DataFrame(columns=['close', 'high', 'low', 'open', 'volume']) for k in
                          ASSETS_MAPPING.keys()}
    ...
    def clean_memory(self):
        for asset in ASSETS_MAPPING.keys():
            self.data_save[asset] = pd.DataFrame(columns=['close', 'high', 'low', 'open', 'volume'])
        return
...
class backTest:
    ...
    def pnl_analyze(self, strategyDetail):
        ...
        balance_hourly.plot(figsize=(12, 5), title='Balance Curve', grid=True)
...
if __name__ == '__main__':
    ...
    strategyDetail.to_csv(os.getcwd() + "/backtest_details.csv")  # output backtest details to your working folder
