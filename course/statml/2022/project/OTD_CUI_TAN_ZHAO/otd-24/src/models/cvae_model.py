...
class CVAE(BaseModel):
    """
        Implements Conditional VAE i.e reconstruction is conditioned on (z,c)

        This is base model to implement reconstruction based ablations of FCRL
    """

    def __init__(self, encoder, decoder, c_type, c_size, z_size, output_distribution,
    ...
    def loss(self, pred, batch, reduce=True):
        ...
        if self.output_distribution == "gaussian":
            distortion = F.mse_loss(
                x_hat.reshape(N, -1), x.reshape(N, -1), reduction="none"
            ).sum(dim=1)
        ...
        return self.lambda_ * distortion + self.beta * kl, {"distortion": distortion, "kl": kl}
