...
class AdvForgettingTrainer(BaseTrainer):
    def __init__(self, model, optimizer: Dict, result_dir, statefile=None, log_every: int = 100,
                 ...
                 evaluations: Optional[Dict] = None, model_update_freq: int = 1, **kwargs):
    ...
    def compute_encoding(self, loader):
        ...
        z_wave = np.zeros((n, self.model.z_size))
        y = np.zeros(n)
        ...
        with torch.no_grad():
            for i, batch in enumerate(loader):
                ...
                z, mask = temp["z"], temp["mask"]
                z_wave[i * batch_size: (i + 1) * batch_size] = (z * mask).cpu().data.numpy()
    ...
    def run_iteration(self, batch, training=True, reduce=True):
        ...
        if update_model and training:
            # randomize c labels if training the model
            N = batch[1].shape[0]
            batch[1] = batch[1][torch.randperm(N)]
        # breakpoint()
    ...
    def test(self, train_loader, test_loader, *args, **kwargs):
        ...
        z_wave_test, c_test, y_test = temp.z_wave, temp.c, temp.y

        logger.info("Computing encodings for train data")
        temp = self.compute_encoding(train_loader)
        z_wave_train, c_train, y_train = temp.z_wave, temp.c, temp.y

        logger.info("Saving encoding")
        np.save(f"{self.result_dir}/embedding.npy", {"z_wave_train": z_wave_train,
                                                     "z_wave_test": z_wave_test,
                                                     "c_train": c_train,
                                                     "c_test": c_test,
                                                     "y_train": y_train,
                                                     "y_test": y_test,
        ...
        if y_train is not None and y_test is not None:
            score, rf_prob = do_classification(z_wave_train, y_train, z_wave_test, y_test,
                                               simple=True)
            ...
            score, nn_prob = do_classification(z_wave_train, y_train, z_wave_test, y_test,
                                               simple=False)
