import math
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from box import Box
from random import shuffle

def get_arch(input_size, z_size, y_size, y_type, c_size, c_type, *args, **kwargs):
    if y_type == "binary":
        y_size = math.ceil(math.log(y_size))
    return Box({"encoder": encoder(input_size, z_size),
                "nce_estimator": CPC(input_size, z_size, c_size,y_size),
                "predictor": predictor(z_size, y_size, y_type, c_size, c_type),
                })


def encoder(input_size, z_size):
    return nn.Sequential(
        nn.Linear(input_size[0], 100),
        nn.ReLU(),
        nn.Linear(100, 2 * z_size),
    )


class CPC_original(nn.Module):
    def __init__(self, input_size, z_size, c_size):
        super().__init__()

        self.f_x = nn.Sequential(
            nn.Linear(input_size[0], 100),
            nn.ReLU(),
            nn.Linear(100, z_size),
        )

        # just make one transform
        self.f_z = nn.Sequential(nn.Linear(z_size, z_size))
        self.w_s = nn.Parameter(data=torch.randn(c_size, z_size, z_size))
        self.device = "cpu"

    def to(self, device):
        self.device = device
        return super().to(device=device)

    def forward(self, x, c, z):
        N = x.shape[0]
        f_x = self.f_x(x)
        f_z = self.f_z(z)

        temp = torch.bmm(
            torch.bmm(
                f_x.unsqueeze(2).transpose(1, 2), self.w_s[c.reshape(-1)]
            ),
            f_z.unsqueeze(2),
        )
        T = F.softplus(temp.view(-1))

        neg_T = torch.zeros(N, device=self.device)

        for cat in set(c.reshape(-1).tolist()):
            f_z_given_c = f_z[(c == cat).reshape(-1)]
            f_x_given_c = f_x[(c == cat).reshape(-1)]
            # weight matrix can be sampled? and the gradient can still be backwarded
            # (N,Z) X (Z,Z)
            temp = F.softplus(
                f_x_given_c @ self.w_s[cat] @ f_z_given_c.transpose(0, 1)
            )
            neg_T[(c == cat).reshape(-1)] = temp.mean(dim=1).view(-1)

        return torch.log(T + 1e-8) - torch.log(neg_T + 1e-8)

class CPC(nn.Module):
    def __init__(self, input_size, z_size, c_size,y_size):
        super().__init__()
        c_size_total=0

        for i in range(len(c_size)):
            c_size_total+=c_size[i]
        self.g_y=nn.Sequential(
            nn.Linear(z_size , 50),
            nn.ReLU(),
            nn.Linear(50, y_size))

        # just make one transform
        # self.f_z = nn.Sequential(nn.Linear(z_size, z_size))
        # self.w_s = nn.Parameter(data=torch.randn(c_size, z_size, z_size))
        self.device = "cpu"

        self.classifier=nn.Sequential(
            nn.Linear(z_size+c_size_total+y_size , 50),
            nn.ReLU(),
            nn.Linear(50, y_size))

    def to(self, device):
        self.device = device
        return super().to(device=device)

    def forward(self, x, c, z,y,idx):

        N=x.shape[0]

        idx_prod=idx[int(N/2):]


        y=y.float()
        y[idx_prod]=self.g_y(z[idx_prod])

        pred=self.classifier(torch.cat((torch.cat((z,c),dim=1),y),dim=1))

        return pred
        # f_x = self.f_x(x)
        # f_z = self.f_z(z)
        #
        # temp = torch.bmm(
        #     torch.bmm(
        #         f_x.unsqueeze(2).transpose(1, 2), self.w_s[c.reshape(-1)]
        #     ),
        #     f_z.unsqueeze(2),
        # )
        # T = F.softplus(temp.view(-1))
        #
        # neg_T = torch.zeros(N, device=self.device)
        #
        # for cat in set(c.reshape(-1).tolist()):
        #     f_z_given_c = f_z[(c == cat).reshape(-1)]
        #     f_x_given_c = f_x[(c == cat).reshape(-1)]
        #     # weight matrix can be sampled? and the gradient can still be backwarded
        #     # (N,Z) X (Z,Z)
        #     temp = F.softplus(
        #         f_x_given_c @ self.w_s[cat] @ f_z_given_c.transpose(0, 1)
        #     )
        #     neg_T[(c == cat).reshape(-1)] = temp.mean(dim=1).view(-1)

        #return torch.log(T + 1e-8) - torch.log(neg_T + 1e-8)


def predictor(z_size, y_size, y_type, c_size, c_type):
    c_size_total=0

    if len(c_size)>1:
        for i in range(len(c_size)):
            c_size_total+=c_size[i]
    else:
        if c_type == "binary":

            c_size_total = math.ceil(math.log(c_size))
        else:
            c_size_total=c_size
    # return nn.Sequential(nn.Linear(z_size + c_size_total, 50), nn.ReLU(), nn.Linear(50, y_size))
    return nn.Sequential(nn.Linear(z_size , 50), nn.ReLU(), nn.Linear(50, y_size))
