...
for test_date in range(198701, 201701, 100):
    ...
    for lamb in [1e-4, 1e-3, 1e-2, 1e-1]:
        ENET_H = make_pipeline(StandardScaler(), SGDRegressor(loss = 'huber', penalty = 'elasticnet', alpha = lamb,  l1_ratio = 0.5, epsilon = 0.05, max_iter = 1e6, shuffle = False))
        ENET_H.fit(x_train, y_train)
        val_pred = ENET_H.predict(x_val).reshape(-1)
    ...
    ENET = SGDRegressor(loss = 'huber', penalty = 'elasticnet', alpha = lamb_max,  l1_ratio = 0.5, epsilon = 0.05, max_iter = 1e6, shuffle = False)
    model_best = make_pipeline(StandardScaler(), ENET)
    model_best.fit(x_all, y_all)

    R2_test = R2_OOS(y_test, model_best.predict(x_test).reshape(-1))
    R2_year[ind] = R2_test
    lamb_year[ind] = lamb_max
    comp_year[ind] = sum(np.abs(ENET.coef_) >= eps)
    ind += 1

    del x_test, y_test, model_best, ENET

ind_max = np.argmax(R2_year)
R2_max = R2_year[ind_max]
lamb_max = lamb_year[ind_max]
pd.DataFrame(R2_year).to_csv('ENETH_R2.csv',  index = False)
pd.DataFrame(comp_year).to_csv('ENETH_comp.csv',  index = False)
...
for name in col_name:
    ...
    ENET_H = make_pipeline(StandardScaler(), SGDRegressor(loss = 'huber', penalty = 'elasticnet', alpha = lamb_max,  l1_ratio = 0.5, epsilon = 0.05, max_iter = 1e6, shuffle = False))
    ENET_H.fit(x_train, y_train)
    R2_name = R2_OOS(y_test, ENET_H.predict(x_test).reshape(-1))
...
Im.columns = col_name
Im.to_csv('ENETH_Importance.csv',  index = False)