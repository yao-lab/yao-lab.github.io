...
for test_date in range(198701, 201701, 100):
    ...
    for K in range(5, 50, 10):
        pls_0 = PLSRegression(n_components = K)
        pls = make_pipeline(StandardScaler(), pls_0)
        pls.fit(x_train, y_train)
        val_pred = pls.predict(x_val).reshape(-1)
    ...
    pls = PLSRegression(n_components = K_cv)
    model_best = make_pipeline(StandardScaler(), pls)
    ...
    del x_test, y_test, pls, model_best

ind_max = np.argmax(R2_year)
R2_max = R2_year[ind_max]
K_max = K_year[ind_max].astype(int)
pd.DataFrame(R2_year).to_csv('PLS_R2.csv',  index = False)
pd.DataFrame(K_year).to_csv('PLS_comp.csv',  index = False)
...
for name in col_name:
    ...
    pls = PLSRegression(n_components = K_max)
    model_best = make_pipeline(StandardScaler(), pls)
    model_best.fit(x_train, y_train)
    R2_name = R2_OOS(y_test, model_best.predict(x_test).reshape(-1))
...
Im.columns = col_name
Im.to_csv('PLS_Importance.csv',  index = False)