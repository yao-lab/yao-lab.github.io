...
% Randomly choose the source/react state
label0 = find(c==0);
label1 = find(c==1);
Acc = zeros(100,1);
for i = 1: 100
    source = label0(randi(length(label0)));
    react = label1(randi(length(label1)));
    % Node source state S(127),  react state S(838）
    [c_TPT,J_plus,T,Acc_TPT] = TPT2(B,c,source,react);
    Acc(i) = Acc_TPT;
end
...
saveas(gcf,'Blog_TPT_Random.jpg')
