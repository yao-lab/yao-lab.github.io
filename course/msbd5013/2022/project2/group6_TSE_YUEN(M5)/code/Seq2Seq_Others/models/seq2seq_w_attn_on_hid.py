...
class Encoder(nn.Module):
    ...
    def forward(self, x, x_emb, x_cal_emb):
        ...
        output_emb = [emb(x_emb[:, :, i].long()) for i, emb in enumerate(self.embeddings)]
        output_emb = torch.cat(output_emb, 2)

        # share embedding layer for both the calendar events
        output_emb_cal = [self.cal_embedding(x_cal_emb[:, :, 0].long()), self.cal_embedding(x_cal_emb[:, :, 1].long())]
