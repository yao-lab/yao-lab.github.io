...
\begin{itemize}
        \item \fromSlide{1}{$\H_K$ can be dense in continuous function space $\C(X)$}
        \item \fromSlide{2}{the gradient takes a simple form in $\H_K$:
        $$\grad_f V = V'_f(y,f(x)) K_x $$}
        \item \fromSlide{3}{super-fast convergence rate for plug-in classifiers...}
\end{itemize}
\end{slide}
}
...
\medskip
\fromSlide{2}{Note: if $K$ is a stochastic density kernel, $L_K^r$ is used to construct diffusion wavelets
by Coifman et al. }
\end{slide}
}
...
\begin{itemize}
    ...
    \fromSlide{3}{\item Suitable for online learning, instead of batch learning.}
\end{itemize}
\end{slide}
}
...
\begin{itemize}
    \fromSlide{2}{\item In fact, $f_t$ can be realized by the Orthogonal Projections on to
